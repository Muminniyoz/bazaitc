...
public class EventHistoryResource {
    ...
    private final EventHistoryQueryService eventHistoryQueryService;

    public EventHistoryResource(EventHistoryService eventHistoryService, EventHistoryQueryService eventHistoryQueryService) {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of eventHistories in body.
     */
    @GetMapping("/event-histories")
    public ResponseEntity<List<EventHistoryDTO>> getAllEventHistories(EventHistoryCriteria criteria, Pageable pageable) {
        log.debug("REST request to get EventHistories by criteria: {}", criteria);
        Page<EventHistoryDTO> page = eventHistoryQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /event-histories/count} : count all the eventHistories.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/event-histories/count")
    public ResponseEntity<Long> countEventHistories(EventHistoryCriteria criteria) {
        log.debug("REST request to count EventHistories by criteria: {}", criteria);
        return ResponseEntity.ok().body(eventHistoryQueryService.countByCriteria(criteria));
    }

    /**
     * {@code GET  /event-histories/:id} : get the "id" eventHistory.
     *
     * @param id the id of the eventHistoryDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the eventHistoryDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/event-histories/{id}")
    public ResponseEntity<EventHistoryDTO> getEventHistory(@PathVariable Long id) {
        log.debug("REST request to get EventHistory : {}", id);
